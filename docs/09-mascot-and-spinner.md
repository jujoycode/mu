...
캐릭터 컨셉 후보 (도그푸딩 초기에 팀 투표로 확정):

1. **엔소(円相, ⟳) 계열** — 선(禪)의 원. 붓으로 그린 열린 원 한 획.
   미니멀리즘·無 정체성에 가장 잘 맞고, 블록 문자로 3행 표현 가능.
2. **μ 글리프 의인화** — 그리스 문자 μ에 눈. pi 계보(π→μ) 계승을 시각화.
3. **점(点) 하나** — 극단적 미니멀. `·`가 눈을 뜨는 수준.

추천: **1안(엔소)** 기본 + μ를 서명처럼 곁들임. 무채색 계열(mu accent)로,
Clawd의 오렌지와 차별화.

**기술 구조는 그대로 채택** (자체 아트로):
- `src/tui/mascot/Mascot.tsx` — 포즈별 세그먼트 방식(바뀌는 부분만 교체, 폭 고정)
- `src/tui/mascot/AnimatedMascot.tsx` — Frame 배열 + `hold(pose, offset, n)` 헬퍼,
  60ms 재생, idle 복귀, **클릭 시 랜덤 시퀀스** (Ink `onClick`)
- 최소 2포즈로 시작(`default` / `active`), 애니메이션 1종(예: 원이 한 번 완성되는
  brush 시퀀스). reduced-motion·비대화형(`-p`)에서는 정적 1프레임.
